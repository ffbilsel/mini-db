...
int dm_page_read(char *buffer, uint32_t page_id) {
    ...
    if (fseek(dm_reader, (long)page_id * DM_PAGE_SIZE, SEEK_SET) != 0) return 1;
    if (fread(buffer, DM_PAGE_SIZE, 1, dm_reader) != 1) return 1;
    return 0;
}
...
int dm_page_write(char *buffer, uint32_t page_id, int is_append) {
    ...
    if (is_append == 1) {
        ...
        seek_result = fseek(dm_writer, (long)page_id * DM_PAGE_SIZE, SEEK_SET);
    }
    if (seek_result != 0) return 1;

    if (fwrite(buffer, DM_PAGE_SIZE, 1, dm_writer) != 1) return 1;
...
int dm_init(void) {
    ...
    if (dm_writer != NULL && dm_reader != NULL) {
        ...
        dm_page_count = ftell(dm_reader) / DM_PAGE_SIZE;
        return 0; 
    }
    return 1; 
}
